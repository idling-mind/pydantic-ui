...
class UnionExampleConfig(BaseModel):
    ...
    current_shape: Union[Circle, Rectangle, Triangle] = Field(
        default_factory=lambda: Circle(),
        description="Currently selected shape (no discriminator, type inferred)"
    )
    
    # Array of Regular Union
    shapes: list[Union[Circle, Rectangle, Triangle]] = Field(
        default_factory=lambda: [Circle(), Rectangle()],
    ...
    identifier: Union[str, int] = Field(
        default="ID-001",
        description="Can be either a string ID or numeric ID"
    )
    
    # Optional Union
    backup_pet: Union[Cat, Dog, None] = Field(
        default=None,
        description="Optional backup pet"
    )
